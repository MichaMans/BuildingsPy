...
class Simulator(object):
    ...
    :param tempDir: An optional path where temporary files will be stored.
...
#        fileMo = os.path.abspath(os.path.join(packagePath, "package.mo"))
    ...
    def _get_dymola_commands(self, working_directory, log_file, model_name,
                             translate_only=False, multi_mode='single'):
        ...
        '''
        if multi_mode== 'multi_append' or multi_mode == 'multi_execute':
            s = """
...
""".format(start_time=self._simulator_.get('t0'),
    ...
    def translate(self,multi_mode='single',prescDir=None):
        ...
        is on the system PATH variable. If it is not found, the function returns with
        an error message.
        This method allows translation of single (multi_mode='single',
        default) instances or multiple (multi_mode='multi_start',
        'multi_append','multi_execute') instances. For multiple instances,
        use 'multi_start' to write a new .mos-file, append instances with
        'multi_append' and append and execute the .mos-file with
        'multi_execute'.
        ...
        try:
            ...
            if prescDir != None:
                ...
                transDir = worDir
            #print('Model to translate: ', self.modelName)
            #print('Relates to master file: ', transDir)
            #print('Translated models goes here: ', worDir)
            #print('_translateDir: ', self._translateDir_)
            runScriptName = os.path.join(transDir, "run_translate.mos")
            if multi_mode == "multi_append" or multi_mode == "multi_execute":
                fil=open(runScriptName, "a")
            else:
                fil=open(runScriptName, "w")
            fil.write(self._get_dymola_commands(
                working_directory=worDir,
            ...
            # Run translation
            if multi_mode == "single" or multi_mode == "multi_execute":
                self._runSimulation(runScriptName,
                                self._simulator_.get('timeout'),
                                worDir)
            return worDir
    ...
    def simulate_translated(self):
        ...
        worDir = self._create_worDir()
        self._simulateDir_ = worDir
        #print('Model to simulate: ', self.modelName)
        #print('worDir: ',worDir)
        #print('transDir: ',self._translateDir_)
        ...
        try:
            ...
            print('Simulator file is written to: ',runScriptName)
            print('Working Dir for Simulation is: ',worDir)
            fil=open(runScriptName, "w")
...
""".format(self._simulator_.get('resultFile') + ".mat")
    ...
    def _copyResultFiles(self, srcDir):
        ...
        for fil in filLis:
            ...
            try:
                print('Result file is copied from: ',srcFil)
                print('Result file is copied to: ',newFil)
                if os.path.exists(srcFil):
                    shutil.copy(srcFil, newFil)
                else:
                    print(srcFil,' does not exist.')
            except IOError as e:
...
#                                        str(staTim))
    ...
    def _create_worDir(self):
        ...
        dirNam=ds[len(ds)-1]
        worDir = os.path.join(tempfile.mkdtemp(prefix='tmp-simulator-' +
                                                      getpass.getuser() +
                                                      '-',dir=self.tempDir),
                                                      dirNam)
